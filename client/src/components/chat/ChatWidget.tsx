...
const suggestionChips = [
    ...
    // subscriptions removed on purpose
]
...
const initialMessage: ChatMessage = {
    ...
    author: "assistant",
    content:
        "Hey there! I can spot rising spend or draft budgets. Want to try one?",
...
// Extend the UI message to optionally carry structured payloads
type ChatAnyMessage = ChatMessage & {
    payload?: any
}
...
function TypingDots() {
    return (
        <div className="max-w-[80%] rounded-3xl bg-primary/10 p-3">
      <span className="inline-flex items-center gap-1">
        <span className="h-2 w-2 rounded-full bg-foreground/60 animate-bounce [animation-delay:-200ms]" />
        <span className="h-2 w-2 rounded-full bg-foreground/60 animate-bounce [animation-delay:-100ms]" />
...
export function ChatWidget() {
    ...
    const [messages, setMessages] = useState<ChatAnyMessage[]>([initialMessage])
    ...
    useEffect(() => {
        ...
        const handleMandate = (event: Event) => {
            ...
            const assistantMessage: ChatAnyMessage = {
    ...
    const updateMandateOnMessage = (mandateId: string, updater: (mandate: MandateAttachment) => MandateAttachment) => {
        setMessages((prev) =>
            prev.map((message) => {
                if (!message.mandate || message.mandate.id !== mandateId) return message
    ...
    const handleSend = (raw: string) => {
        ...
        chatMutation.mutate(
            ...
            {
                onSuccess: (data) => {
                    // data is NormalizedChat from useChat
                    const { replyText, payload, timestamp } = data

                    if (payload) {
                        // Render structured card/insight
                        const assistantMessage = {
                            ...createAssistantMessage(""),
                            timestamp,
                            // put structured result on the message
                            payload,
                        } as any
                        setMessages((prev) => [...prev, assistantMessage])
                        return
                    ...
                    // Plain text path — ONLY the reply string
                    const text = replyText?.trim()
                    const assistantMessage = createAssistantMessage(
                        text && text.length > 0
                            ? text
                            : "Flow Coach is still thinking—try asking again in a moment.",
                        timestamp
                    )
                    setMessages((prev) => [...prev, assistantMessage])
                },
    ...
    return (
        <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="sm:max-w-xl">
                <SheetHeader>
                    <SheetTitle>Flow Coach</SheetTitle>
                    <SheetDescription>Gemini-powered tips grounded in your Swipe Coach data.</SheetDescription>
                </SheetHeader>

                <div className="mt-4 flex h-[calc(100vh-9rem)] flex-col gap-4">
                ...
                <span className="flex h-9 w-9 items-center justify-center rounded-2xl bg-primary/20 text-primary">
                  <MessageCircle className="h-4 w-4" />
                        ...
                        <CardContent className="flex min-h-0 flex-1 flex-col gap-4">
                            ...
                            <div
                                ref={listRef}
                                className="glass-panel flex-1 min-h-0 space-y-4 overflow-y-auto rounded-3xl border border-border/60 bg-white/70 p-6 dark:bg-zinc-900/60"
                                style={{ maxHeight: "calc(100vh - 240px)" }}
                            >
                                {messages.map((message) => {
                                    // 1) Mandate messages (text + card)
                                    if ((message as any).mandate) {
                                        return (
                                            <div key={message.id} className="flex flex-col items-start gap-3">
                                                {message.content ? (
                                                    <div className="max-w-[80%] rounded-3xl bg-primary/10 p-4 text-sm prose prose-sm dark:prose-invert">
                                                ...
                                                <MandateCard
                                                    mandate={(message as any).mandate}
                                                    onApprove={() => handleApprove((message as any).mandate!)}
                                                    onDecline={() => handleDecline((message as any).mandate!)}
                                                    isProcessing={actionState?.id === (message as any).mandate.id}
                                                />
                                            </div>
                                        )
                                    }

                                    // 2) Structured payloads from the server (render custom components)
                                    const payload = (message as any).payload
                                    if (payload?.type === "best_card.result") {
                                        return (
                                            <div key={message.id} className="flex max-w-[90%] flex-col items-start">
                                                <ChatBestCard payload={payload} />
                                            </div>
                                        )
                                    }
                                    if (payload?.type?.startsWith?.("insight.")) {
                                        // subscriptions intentionally not used
                                        if (payload.type === "insight.subscriptions") return null
                                        return (
                                            <div key={message.id} className="flex max-w-[90%] flex-col items-start">
                                                <ChatInsights payload={payload} />
                                            </div>
                                        )
                                    }

                                    // 3) Normal text bubbles
                                    // Normal text bubbles with Markdown for assistant
                                    return (
                                        <div
                                            key={message.id}
                                            className={
                                                message.author === "assistant"
                                                    ? "max-w-[80%] rounded-3xl bg-primary/10 p-4 text-sm prose prose-sm dark:prose-invert"
